...
FOSSIL_TEST(cpp_soap_detect_known_id) {
    ASSUME_ITS_TRUE(Soap::detect("This is outrageous and infuriating!", "ragebait", "sentence"));
}

FOSSIL_TEST(cpp_soap_detect_unknown_id_safe) {
    ASSUME_ITS_FALSE(Soap::detect("Normal text.", "nonexistent_detector", "sentence"));
}

FOSSIL_TEST(cpp_soap_detect_empty_text_safe) {
    ASSUME_ITS_FALSE(Soap::detect("", "spam", "sentence"));
}
...
FOSSIL_TEST(cpp_soap_analyze_grammar_style_basic) {
    auto r = Soap::analyze_grammar_style("The ball was thrown by the boy.");
...
FOSSIL_TEST(cpp_soap_correct_grammar_basic) {
    std::string out = Soap::correct_grammar("I should of gone to the store.");
...
FOSSIL_TEST(cpp_soap_readability_label_bounds) {
    ASSUME_ITS_TRUE(Soap::readability_label(0).size() > 0);
    ASSUME_ITS_TRUE(Soap::readability_label(100).size() > 0);
}

FOSSIL_TEST(cpp_soap_sanitize_basic) {
    std::string out = Soap::sanitize("You are an idiot!");
    ASSUME_ITS_TRUE(out.size() > 0);
}

FOSSIL_TEST(cpp_soap_suggest_basic) {
    std::string out = Soap::suggest("He got that rizz.");
    ASSUME_ITS_TRUE(out.size() > 0);
}

FOSSIL_TEST(cpp_soap_summarize_basic) {
    std::string out = Soap::summarize("This paragraph explains something in great detail over many sentences that are unnecessary.");
    ASSUME_ITS_TRUE(out.size() > 0);
}

FOSSIL_TEST(cpp_soap_split_sentences) {
    auto parts = Soap::split("Hello world. This is Fossil.", "sentence");
...
FOSSIL_TEST(cpp_soap_reflow_basic) {
    std::string out = Soap::reflow("This is a long sentence that should wrap.", 15);
    ASSUME_ITS_TRUE(out.find('\n') != std::string::npos);
}

FOSSIL_TEST(cpp_soap_normalize_basic) {
    std::string out = Soap::normalize("This   is   spaced.");
    ASSUME_ITS_EQUAL_CSTR(out.c_str(), "This is spaced.");
}

FOSSIL_TEST(cpp_soap_capitalize_upper) {
    std::string out = Soap::capitalize("hello world", 2);
    ASSUME_ITS_EQUAL_CSTR(out.c_str(), "HELLO WORLD");
}

FOSSIL_TEST(cpp_soap_process_no_options) {
    std::string out = Soap::process("Simple text.", "sentence", nullptr);
    ASSUME_ITS_TRUE(out.size() > 0);
}

FOSSIL_TEST(cpp_soap_process_with_options) {
    Soap::Options opt;
    opt.detect_spam(true).include_flags(true).include_summary(true);

    std::string out = Soap::process("Buy now, limited time offer!", "marketing", &opt);
    ASSUME_ITS_TRUE(out.size() > 0);
}

FOSSIL_TEST(cpp_soap_all_empty_inputs_safe) {
    Soap::Options opt;
    ASSUME_ITS_TRUE(Soap::sanitize("").size() == 0);
