...
int fossil_io_cstring_string_to_money(const char *input, double *amount)
{
    ...
    {
        if (buffer[i] == decimal_sep)
            buffer[i] = '.';
    }

    *amount = atof(buffer);
    if (negative || strchr(input, '-'))
        *amount = -*amount;
...
int fossil_io_cstring_money_to_string_currency(double amount, char *output, size_t size, const char *currency)
{
    ...
    if ((size_t)(total_len + strlen(currency) + 2) > size)
        return -1;
...
{
    if (!input || !needle || !*needle || !replacement)
        return NULL;
...
int fossil_io_cstring_piglatin(const char *input, char *output, size_t size)
{
    ...
    while (token)
    {
        ...
        if (out_len + word_space + extra + 1 > size)
            return -1;
        if (!first_word)
        {
            output[out_len] = ' ';
...
static int fossil_io_cstring_number_to_words_inner(int num, char *buffer, size_t size)
{
    ...
    if (num >= 100)
    {
        ...
        if (hundreds < 0 || hundreds > 19)
            return -1;
        if (used + strlen(units[hundreds]) + 8 >= size)
            return -1;
    ...
    else if (num > 0 || used == 0)
    {
        if (num < 0 || num > 19)
            return -1;
        if (used + strlen(units[num]) + 1 >= size)
            return -1;
        strcat(buffer, units[num]);
    }
    return 0;
...
cstring fossil_io_cstring_zalgo(ccstring str)
{
    ...
    // Zalgo combining marks (Unicode)
    static const char *zalgo_up[] = {
        "\u030d", "\u030e", "\u0304", "\u0305", "\u033f", "\u0311", "\u0306",
        "\u0310", "\u0352", "\u0357", "\u0351", "\u0307", "\u0308", "\u030a",
        "\u0342", "\u0343", "\u0344", "\u034a", "\u034b", "\u034c", "\u0303",
        "\u0302", "\u030c", "\u0350", "\u0300", "\u0301", "\u030b", "\u030f",
    ...
    size_t len = strlen(str);
    // Estimate: each char + up to 8 marks, each mark up to 4 bytes, plus null
    size_t max_out = len * (1 + 8 * 4) + 1;
    ...
    size_t out_pos = 0;
    for (size_t i = 0; i < len && out_pos < max_out - 8; i++)
    {
        ...
        for (int j = 0; j < up && out_pos < max_out - 8; j++)
        {
            const char *mark = zalgo_up[rand() % (sizeof(zalgo_up) / sizeof(zalgo_up[0]))];
            size_t mlen = strlen(mark);
            memcpy(out + out_pos, mark, mlen);
            out_pos += mlen;
        }
        for (int j = 0; j < mid && out_pos < max_out - 8; j++)
        {
            const char *mark = zalgo_mid[rand() % (sizeof(zalgo_mid) / sizeof(zalgo_mid[0]))];
            size_t mlen = strlen(mark);
            memcpy(out + out_pos, mark, mlen);
            out_pos += mlen;
        }
        for (int j = 0; j < down && out_pos < max_out - 8; j++)
        {
            const char *mark = zalgo_down[rand() % (sizeof(zalgo_down) / sizeof(zalgo_down[0]))];
            size_t mlen = strlen(mark);
            memcpy(out + out_pos, mark, mlen);
            out_pos += mlen;
        }
...
int fossil_io_cstring_number_to_words(int num, char *buffer, size_t size)
{
    ...
    if (num >= 1000)
    {
        ...
        if (strlen(buffer) + strlen(units[thousands]) + 10 >= size)
            return -1;
    ...
    {
        if (fossil_io_cstring_number_to_words_inner(num, buffer, size) != 0)
            return -1;
    }
...
{
    for (int i = 0; i < 20; i++)
        if (strcmp(word, units[i]) == 0)
            return i;
    for (int i = 2; i < 10; i++)
        if (strcmp(word, tens[i]) == 0)
            return i * 10;
...
cstring fossil_io_cstring_trim_safe(ccstring str, size_t max_len)
{
    ...
    // Trim left
    size_t start = 0;
    while (start < max_len && isspace((unsigned char)copy[start]))
        start++;

    // Trim right
    size_t end = strnlen(copy, max_len);
    while (end > start && isspace((unsigned char)copy[end - 1]))
        end--;
...
cstring *fossil_io_cstring_split_safe(ccstring str, char delimiter, size_t *count, size_t max_len)
{
    ...
    for (size_t i = 0; i <= str_len; i++)
    {
        if (str[i] == delimiter || str[i] == '\0')
        {
            ...
            if (len > str_len - (start - str))
                len = str_len - (start - str);
            if (len > max_len)
                len = max_len;
...
cstring fossil_io_cstring_to_upper_safe(ccstring str, size_t max_len)
{
    if (!str)
        return NULL;
    cstring copy = fossil_io_cstring_copy_safe(str, max_len);
    for (size_t i = 0; i < strnlen(copy, max_len); i++)
...
cstring fossil_io_cstring_substring_safe(ccstring str, size_t start, size_t length, size_t max_len)
{
    ...
    size_t str_len = strnlen(str, max_len);
    if (start >= str_len)
        return fossil_io_cstring_create_safe("", max_len);
...
cstring fossil_io_cstring_pad_left_safe(ccstring str, size_t total_length, char pad_char, size_t max_len)
{
    ...
    if (len >= total_length)
        return fossil_io_cstring_substring_safe(str, 0, total_length, max_len);

    cstring result = fossil_io_cstring_create_safe("", max_len);
    char pad_str[2] = {pad_char, '\0'};
    for (size_t i = 0; i < total_length - len; i++)
        fossil_io_cstring_append_safe(&result, pad_str, max_len);
    fossil_io_cstring_append_safe(&result, str, max_len);
    return result;
}
...
cstring fossil_io_cstring_pad_right_safe(ccstring str, size_t total_length, char pad_char, size_t max_len)
{
    ...
    char pad_str[2] = {pad_char, '\0'};
    for (size_t i = len; i < total_length; i++)
        fossil_io_cstring_append_safe(&result, pad_str, max_len);
    return result;
    return result;
}
...
int fossil_io_cstring_ends_with_safe(ccstring str, ccstring suffix, size_t max_len)
{
    ...
    size_t len_str = strnlen(str, max_len);
    if (len_suffix == 0 || len_suffix > len_str)
        return 0;
