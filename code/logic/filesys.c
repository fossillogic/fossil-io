...
{
    if (!path || !hash_out || !algorithm) return -1;
    ...
    uint32_t strategy)
{
    ...
    else if (strategy == FOSSIL_MERGE_INTERLEAVE)
    {
        ...
        do {
            ...
            if (n1) fwrite(buf1, 1, n1, out);

            n2 = fread(buf2, 1, sizeof(buf2), b);
            if (n2) fwrite(buf2, 1, n2, out);
